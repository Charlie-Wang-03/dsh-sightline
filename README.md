...
- DSH host tool: argument-free `sightline` tool binds observed evidence to `exec.agent.session`, resolves the same `cwd` for all three agents, returns the canonical JSON report, and renders a compact three-column matrix from that same report.
- Pure cross-agent comparison: preserves `present`, `absent`, and `unknown` as distinct states.
- GitHub Actions baseline: typecheck, build, resolver tests, DSH provenance integration, host/tool integration, and the first divergent three-column report fixture.
...
live DSH tool call + workspace
            ...
            |                                                              |
            +---- local filesystem ------> Codex adapter ---- Predicted ----+--> canonical report
            |                                                              |
            +---- local filesystem ------> Claude adapter --- Predicted ----+
                                                                           |
                                                                           +--> tool JSON value
                                                                           +--> compact matrix
```
...
- [`docs/DSH_HOST_TOOL.md`](docs/DSH_HOST_TOOL.md) — tool ownership, workspace binding, and the first real three-column report.
...
The current DSH observed and host/tool integration baseline is DeepSeek Harness `0.1.1-rc.2` at commit `b150a551b8d465e31e418e1b2eaf5e79bbb7d28e`. DSH is a developer preview, so every later bundle/client integration step must re-verify the public seams it binds to.
